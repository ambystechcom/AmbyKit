...
---
<!-- GENERATED by AmbyKit from src/prompts/plan.md — do not edit by hand; run `ambykit sync`. -->

Act as the **Architect** in `@.amby/roles/architect.md` and say so in your first line. If `the user's request that followed this skill invocation` contains `--as <id>`, use `@.amby/roles/<id>.md` instead; if that file is missing, stop and list the files in `.amby/roles/`.

Write the implementation plan for feature `the user's request that followed this skill invocation` (default: the current feature).

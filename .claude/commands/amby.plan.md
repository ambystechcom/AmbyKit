...
Act as the **Architect** in `@.amby/roles/architect.md` and say so in your first line. If `$ARGUMENTS` contains `--as <id>`, use `@.amby/roles/<id>.md` instead; if that file is missing, stop and list the files in `.amby/roles/`.
